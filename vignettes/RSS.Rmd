...
nrcadams::docket_codex |>
  ...
  dplyr::mutate(`RSS File` = paste0(
    "https://amritpatel.github.io/nrcadams/RSS/",
    Project |> stringr::str_replace_all(" ", "_"),
    ".rss"
  )) |>
  dplyr::add_row(
    Project = "All Dockets", 
    `RSS File` = "https://amritpatel.github.io/nrcadams/RSS/dockets.rss"
    ) |>
...
nrcadams::RTR_docket_codex |>
  ...
  dplyr::add_row(
    Project = "All Dockets", 
    `RSS File` = "https://amritpatel.github.io/nrcadams/RSS/dockets-RTR.rss"
    ) |>
  nrcadams::hyperlink_file_to_name(name_vct = `RSS File`, url_vct = `RSS File`)|>
  DT::datatable(escape = FALSE)
...
```{r, fuel-vendors}
vendors <- c(99902041)
framatome <- vendors |> nrcadams::search_docket(c(99902041), days_back = 20)

if (length(framatome) != 0) {
  framatome |>
    nrcadams::write_rss(
      file = here::here("docs", "RSS", paste0(
        "framatome", ".rss")
...
tibble::tibble(Project = "Framatome") |>
  dplyr::mutate(`RSS File` = paste0(
    "https:/amritpatel.github.io/nrcadams/RSS/",
    Project,
    ".rss"
